<script setup>
  ...
  import { CheckCircleIcon, XCircleIcon, ChevronRightIcon, ChevronLeftIcon } from "@heroicons/vue/24/solid";
  ...
  <template>
    <div class="w-full h-full">
      ...
      >
        ...
        <div class="w-full h-full flex flex-column items-center justify-center gap-3">
          <div 
            class="hidden ssm:flex items-center h-64 cursor-pointer" 
            :style="{visibility: activeStep !== 'info' ? 'visible' : 'hidden'}" 
            @click="setStep(-1)"
          >
          <ChevronLeftIcon class="h-16 w-16"/>
          </div>
            ...
            >
              <FormKit type="group" id="description" name="description">
                ...
                <div class="flex flex-col xl:flex-row w-full h-full pt-2">
                      <MarkdownEditor
                        :preview="false"
                        class="!h-64 md:!h-96 !w-screen ssm:!w-full"
                        v-model="form.description"
                      />
                      <MarkdownEditor
                        :previewOnly="true"
                        class="!h-64 md:!h-96 !w-screen ssm:!w-full !p-5 !mt-2 xl:!mt-0"
                        v-model="form.description"
                      />
                </div>
                <FormKit
          ...
          <div 
            class="hidden ssm:flex items-center h-64 cursor-pointer" 
            :style="{visibility: activeStep !== 'description' ? 'visible' : 'hidden'}" 
            @click="setStep(1)"
          >
          <ChevronRightIcon class="h-16 w-16 cursor-pointer"/>
          </div>
